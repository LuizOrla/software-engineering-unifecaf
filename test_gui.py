...
async def test_fluxo_inicial_e_dialogos(user: User):
    # Abre a página do app rodando em segundo plano
    await user.open('/')
    
    # Verifica se o cabeçalho e o menu lateral foram renderizados
    await user.should_see('Testando App')
    await user.should_see('Menu')
    await user.should_see('Projetos Salvos')

    # Testa o clique no botão "Nova Tarefa" e valida se a janela de diálogo abriu
    await user.click('Nova Tarefa')
    await user.should_see('Nome da Tarefa')
    await user.should_see('Criar Tarefa')
    
    # Clica em cancelar para fechar o diálogo
...
async def test_aviso_criar_tarefa_sem_projeto(user: User):
    ...
    # Valida se a função gui.nova_tarefa disparou a notificação de erro corretamente
